...
K2_per_step = 10000  # ADADELTA iterations per temperature stage; total ≈ K2
...
alpha_post = 1.0 + Y11_np.flatten()
beta_post  = 1.0 + Y12_np.flatten() - Y11_np.flatten()
_n_total = S * 13
_n_capped = 0
eta_prior_var = 100
for i in range(S):
...
plt.savefig('HPV-conflict-check.pdf')
...
#ax2.set_xlim(13, 17)
_kde_ymax = ax1.get_ylim()[1]
ax1.set_ylim(0, _kde_ymax)
ax2.set_ylim(0, _kde_ymax)
...
plt.savefig('HPV-conflict-check-broken.pdf')
